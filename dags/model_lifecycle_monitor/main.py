...
from dags.model_lifecycle_monitor.repositories.postgres.model_deployments import has_any_active_model
from dags.model_lifecycle_monitor.services.tasks import drift_check, has_drift, trigger_challenger_model_rotation, trigger_training_approval_dispatch
...
@dag(
    dag_id=DagIDs.MODEL_LIFECYCLE_MONITOR,
    schedule="0 */6 * * *",
    ...
    tags=["mle", "monitor", "lifecycle", "orchestrator"]
)
    ...
    def steps()
        ...
        has 2 workflow DESC
            ...
            is workflow promotion_pending
                assert should not happen
    """
    trigger_challenger_model_rotation \
    >> has_any_active_model() >> [
        drift_check \
        >> has_drift() >> [
            trigger_training_approval_dispatch,
            no_action()
        ],
        trigger_training_approval_dispatch
    ]

    # BELOW is trigger_training_approval_dispatch
